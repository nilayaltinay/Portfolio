...
$errors = array(); // Hataları saklamak için boş bir dizi
$success_message = '';
...
if (isset($_POST['send'])) {
   ...
   if (empty($number)) {
      ...
      $errors[] = "Number field should be numeric.";
   }
   ...
   // Hataları kontrol et ve işlem yap
   if (count($errors) > 0) {
      ...
      $stmt = mysqli_prepare($conn, "SELECT * FROM `contact_form` WHERE name = ? AND email = ? AND number = ? AND message = ?");
      ...
      if (mysqli_stmt_num_rows($stmt) > 0) {
         ...
         $stmt = mysqli_prepare($conn, "INSERT INTO `contact_form`(name, email, number, message) VALUES(?, ?, ?, ?)");
         mysqli_stmt_bind_param($stmt, "ssss", $name, $email, $number, $msg);
         mysqli_stmt_execute($stmt);
